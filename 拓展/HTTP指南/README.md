...
# 状态码
 ...
 # GET和POST的区别
      ...
      1：提交方式
          ...
          GET: 请求的参数会附在URL(以"?"分隔URL与传输数据，以"&"连接多个参数)。
          
          POST: 把提交的数据放置在HTTP包的包体中。
          
          so: GET提交的数据会在地址栏显示出来，而POST提交，地址栏不会改变
          
      2：传输数据大小
   
         GET：特定浏览器和服务器对URL长度有限制，例如：IE对url长度的限制是2083字节。对于其他浏览器，理论上没有长度限制，其限制取决于操作系统的支持
         
         POST：由于不是通过URL船只，理论上数据不受限。
         
         so: GET提交时会有数据大小限制(源于URL长度限制),POST提交数据是没有限制
         
      3：安全性
      
         GET：GET提交数据时，其数据会明文出现在url上，也有可能被浏览器缓存。若其他人查看浏览器的历史记录时，就可以看到你提交的数据信息了。
              除此之外，GET提交数据还可能会遭到Cross-site request forgery攻击。
            
         POST：没有GET这些漏洞
         
         so: POST比GET安全性更高
         
      4: get,post都是在http上运行的
      
         GET: 请求参数作为key/value对的序列(查询字符串)附加到URL上的，查询字符串的长度受到web浏览器的限制，不适合传输大型数据，且不安全。
         
         POST: 请求参数是在HTTP标题的一个不同部分(entity body)传输的，这一部分用来传输表单信息。因此必须将Content-type设置为:application/x
               -www-form-urlencoded。post设计用来支持web窗体上的用户字段，其参数也是作为key/value对传输。但是：他不支持复杂数据类型。因为
               post没有定义算数数据结构的语义和规则。
